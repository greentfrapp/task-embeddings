...
class CNN_cifar(object):
	...
	def build_model(self, layers=3, input_tensors=None, noise=False):
		...
		self.train_running_output = train_running_output = tf.reshape(net.output, [batchsize, -1, 2 * 2 * 64])
		...
		# train_running_output = tf.concat([train_running_output, tf.reshape(tf.cast(tf.argmax(self.train_labels, axis=1), dtype=tf.float32), [batchsize, -1, 1])], axis=2)
		...
		with tf.variable_scope("attention"):
			...
			for i in np.arange(3):
				output_weights, _ = self.attention(
					query=output_weights,
					key=output_weights,
				...
				dense = tf.layers.dense(
					...
					activation=tf.nn.relu,
					kernel_initializer=tf.contrib.layers.xavier_initializer(),
					name="encoder2_layer{}_dense1".format(i + 1),
				)
				# if i == 4:
				# 	dense = tf.layers.dropout(
				# 		inputs=train_embed,
				...
				output_weights += tf.layers.dense(
					...
					activation=None,
					kernel_initializer=tf.contrib.layers.xavier_initializer(),
					name="encoder2_layer{}_dense2".format(i + 1)
				)
				# train_embed = tf.layers.dropout(
				# 	inputs=train_embed,
				# 	rate=0.2,
				# 	training=self.is_training,
				# )
				output_weights = tf.contrib.layers.layer_norm(output_weights, begin_norm_axis=2)

			self.output_weights = tf.layers.dense(
				inputs=output_weights,
				# units=(28 - layers) * (28 - layers) * 64,
				units=2 * 2 * 64,
				activation=None,
